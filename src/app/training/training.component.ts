...
export class TrainingComponent implements OnInit {
        ...
        this.trainingService.findAllTrainings(pageNumber).subscribe(
            page => {
                ...
                for (var i = 0; i < this.pagination.content.length; i++)
                    this.pagination.content[i].progress = this.percentageToValue(this.pagination.content[i].progress);
            },
            error => {
                $('#wait').hide();
    ...
    private findProposedTrainings(page: number) {
        $('#wait').show();
        this.trainingService.findProposedTrainings(this.logedUser.id, page).subscribe(
            page => {
                $('#wait').hide();
                this.pagination = page;
                for (var i = 0; i < this.pagination.content.length; i++)
    ...
    private findByMentorIdAndStatus(page: number) {
        $('#wait').show();
        this.trainingService.findByMentorIdAndStatus(this.logedUser.id, this.trainingStatus, page).subscribe(
            page => {
                $('#wait').hide();
                this.pagination = page;
                for (var i = 0; i < this.pagination.content.length; i++)
    ...
    private findByUserIdAndStatus(page: number) {
        $('#wait').show();
        this.trainingService.findByUserIdAndStatus(this.logedUser.id, this.trainingStatus, page).subscribe(
            page => {
                $('#wait').hide();
                this.pagination = page;
                for (var i = 0; i < this.pagination.content.length; i++)
    ...
    private approveTraining(index: number, status: string) {
        $('#wait').show();
        this.trainingService.approveTraining(this.pagination.content[index].id, status).subscribe(
            apiResponse => {
                $('#wait').hide();
    ...
    getTrainingStatus(event: Event) {
        this.selectElementText = event.target['options']
